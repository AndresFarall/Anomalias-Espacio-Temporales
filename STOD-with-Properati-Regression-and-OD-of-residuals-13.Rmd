...
Resid.prim <- prim.box(x=x.data, y=y.data, peel.alpha = 0.5, mass.min = 0.0005, threshold.type=1)
...
# los mas bajos
Resid.prim$y.fun[orden[1:5]]
Resid.prim$mass[orden[1:5]]
# los mas altos
Resid.prim$y.fun[rev(orden)[1:5]]
Resid.prim$mass[rev(orden)[1:5]]

cualmax<-rev(orden)[1]
cualmin<-orden[3]

caja.max<-unlist(Resid.prim$box[cualmax])
caja.min<-unlist(Resid.prim$box[cualmin])

cant.max<-Resid.prim$mass[cualmax]
cant.min<-Resid.prim$mass[cualmin]
...
# Mapa
leaflet(datos.sf) %>% addTiles() %>% addPolygons(data=polc,fillColor = "transparent",opacity = 1,weight = 10,color = "yellow") %>% addCircleMarkers(fillOpacity = 0.5,weight=1,radius=~abs(ResidRelBal*10),color = ~pal(ResidRelBal),popup = ~Desc,label=~paste("Resid",as.character(round(ResidRel,2)))) 
```
